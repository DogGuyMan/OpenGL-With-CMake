 ...
 *  - 타입 불일치 첫 호출만 warn (actual==0 즉 active 정보 없음 경우 skip).
 *  - program 파괴 시 해당 키의 트래커 정리 (@c Invalidate).
 *
 *  ### 설계 — 왜 static (인스턴스 X)
 *  - 본 클래스 자체에 *인스턴스 상태가 필요 없음* — program 별 dedup 은 내부 map 으로.
 *  - `ProgramUniforms` 헤더가 본 헤더를 *include 하지 않아도* 되게 함 → diagnostics 의존성을
 *    cpp 차원으로 가둠 ([architecture.md §4](../../.claude/architecture.md) PRIVATE link 일관).
 *  - 기존 `GLObjectLog::CheckExpectedUniforms` 도 동일 패턴 (static + 내부 program 키 map).
 *
 *  ### 비-책임
 *  - ❌ uniform 값 setter — `SJH::ProgramUniforms` 에서 (program/ 모듈).
 *  - ❌ location 캐싱 — `SJH::ProgramUniforms::Lookup` 에서.
 *
 *  ### Lifecycle
 *  - 호출자(`SJH::Program::~Program()`)가 파괴 시 @c Invalidate(mProgram) 명시 호출 필수.
 *    안 부르면 같은 GLuint 가 재발급될 때 stale 트래커 → 기대 warn 이 silently 묻힐 수 있음.
 */
