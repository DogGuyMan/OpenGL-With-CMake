...
**Spec:** [docs/superpowers/specs/2026-05-07-gl-state-and-test-quality-design.md](../specs/2026-05-07-gl-state-and-test-quality-design.md)
...
namespace SJH::Diagnostics
{
    ...
    // FieldsToString은 Task 6 (Snapshot)에서 채움 — 이 task엔 stub
...
Run: `cmake --build build_Darwin -j && ctest --test-dir build_Darwin --output-on-failure -R "state_fields"`
...
- [ ] **Step 2: 빌드 (테스트 등록 전이라 컴파일 에러 — 다음 step에서 등록)**

먼저 `test/CMakeLists.txt`에 등록. `test_gl_state_fields` 블록 직후:
...
Run: `cmake --build build_Darwin -j && ctest --test-dir build_Darwin --output-on-failure -R "capture"`
...
- [ ] **Step 2: 케이스 1 교체** — `test/test_uniform_diagnostics.cpp:16-31` 의 첫 TEST_CASE
          ...
          "[diagnostics][uniform]")
{
    ...
    REQUIRE(cap.Contains("99"));  // program 핸들 출력 (구현이 program 포함한다고 가정)
...
상단 include 추가 (파일 13행 근처):
```cpp
#include "support/spdlog_capture.h"
```

- [ ] **Step 3: 케이스 2 교체** — `test/test_uniform_diagnostics.cpp:33-49`
...
- [ ] **Step 4: 케이스 3 교체** — `test/test_uniform_diagnostics.cpp:51-66`
...
Run: `cmake --build build_Darwin -j && ctest --test-dir build_Darwin --output-on-failure -R "uniform"`
Expected: 3개 케이스 모두 PASS.

**Note**: Step 2의 마지막 단언 `REQUIRE(cap.Contains("99"))`는 구현이 `program` 핸들을 로그에 포함한다고 가정. 실제 [src/diagnostics/uniform_diagnostics.cpp](../../../src/diagnostics/uniform_diagnostics.cpp)를 읽어 포맷이 다르면 그 단언 조정 (또는 제거).
...
## Task 5: GLStateLog (Production Dump + macOS no-op)

**Goal**: `GLStateLog::Dump(tag)` — production 측 한 줄 덤프. `EnableAutoOnError` — macOS는 std::call_once warn 후 no-op.
...
std::string FieldsToString(const GLStateFields& f)
{
    ...
    auto append = [&](const char* line) { out += line; out += '\n'; };

    // 헤더 — 비대칭 (enum=symbolic, handle=raw)을 한 줄 설명
    append("# GL state (enum=symbolic, handle=raw integer)");
...
Run: `cmake --build build_Darwin -j && ctest --test-dir build_Darwin --output-on-failure -R "state_log"`
Expected: 3개 케이스 모두 PASS.
...
Run: `cmake --build build_Darwin -j && ctest --test-dir build_Darwin --output-on-failure -R "snapshot"`
...
근거: docs/superpowers/specs/2026-05-07-gl-state-and-test-quality-design.md §6.1
...
> 본 문서는 [docs/superpowers/specs/2026-05-07-gl-state-and-test-quality-design.md](../docs/superpowers/specs/2026-05-07-gl-state-and-test-quality-design.md) §6.2 의 운영 산출물.

## 1. 개념

"테스트 통과"가 진짜 안전을 의미하는지 *적대적으로* 검증. 컴포넌트마다 3개의 그럴듯한 사보타지를 손으로 적용하고 ≥1 케이스 FAIL을 강제한다.

근거: testing-curriculum.md §부록 A.5 (적대적 사고) + A 논문 §8 (Mutation Score caveat).
...
매번 `cmake --build && ctest -R snapshot` 실행 후 `git checkout`. 각 결과를 [doc/test-quality-drill/diff.md](../../doc/test-quality-drill/diff.md) 에 기록.
...
매번 빌드/테스트 후 [doc/test-quality-drill/symbolic_name.md](../../doc/test-quality-drill/symbolic_name.md) 갱신.
...
매번 빌드/테스트 후 [doc/test-quality-drill/snapshot_tostring.md](../../doc/test-quality-drill/snapshot_tostring.md) 갱신.
...
| 7 | 구현 순서 9 step | Task 1-9 (Step 매핑: spec Step 1=Task 1+2, Step 2=Task 5(log), Step 3=Task 3, Step 4=Task 4, Step 5=Task 6, Step 6=Task 5+6 통합, Step 7=Task 7, Step 8=Task 8, Step 9=Task 9) |
...
- "Similar to Task N": 0건 (Task 9의 Step 5/6/7은 사보타지 표를 직접 참조하므로 OK).
...
## Plan complete and saved to `docs/superpowers/plans/2026-05-07-gl-state-and-test-quality-implementation.md`.

### Two execution options:

**1. Subagent-Driven (recommended)** — I dispatch a fresh subagent per task, review between tasks, fast iteration. *단, 사용자의 [auto memory phase-implementation-mode 정책](../../../.claude/...)에 따라 코드 작성은 사용자 직접 — 이 옵션은 본 프로젝트와 정책 충돌.*

**2. Inline Execution** — Execute tasks in this session using executing-plans, batch execution with checkpoints for review. *동일 정책 충돌.*

**3. Plan-as-Reference (이 프로젝트의 권장)** — 사용자가 본 plan을 reference로 직접 구현. Claude는 명시 요청 시 Task별 review/문제 발견만. 매 Task 완료 후 ctest 결과 + 발견된 이슈를 Claude에 공유 → Claude가 review.
